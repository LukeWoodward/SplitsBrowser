...
(function () {
    ...
    QUnit.test("Splits graph selector returns result data adjusted to reference, in units of minutes", assert => {
        const chartType = ChartTypes.get("SplitsGraph");
        const result = fromCumTimes(1, 10 * 3600, [0, 65, 221, 409, 578], {});
        const referenceCumTimes = [0, 58, 224, 381, 552];
        assert.deepEqual(chartType.dataSelector(result, referenceCumTimes), [0, (65 - 58) / 60, (221 - 224) / 60, (409 - 381) / 60, (578 - 552) / 60]);
    });

    QUnit.test("Splits graph selector returns result data with a missed control adjusted to reference, in units of minutes", assert => {
        const chartType = ChartTypes.get("SplitsGraph");
        const result = fromCumTimes(1, 10 * 3600, [0, 65, 221, null, 578], {});
        const referenceCumTimes = [0, 58, 224, 381, 552];
        assert.deepEqual(chartType.dataSelector(result, referenceCumTimes), [0, (65 - 58) / 60, (221 - 224) / 60, null, (578 - 552) / 60]);
    });

    QUnit.test("Race graph selector returns result data adjusted to reference with start time added, in units of minutes", assert => {
        const chartType = ChartTypes.get("RaceGraph");
        const result = fromCumTimes(1, 10 * 3600, [0, 65, 221, 409, 578], {});
        const referenceCumTimes = [0, 58, 224, 381, 552];
    ...
    QUnit.test("Race graph selector returns result data with a missed control adjusted to reference with start time added, in units of minutes", assert => {
        const chartType = ChartTypes.get("RaceGraph");
        const result = fromCumTimes(1, 10 * 3600, [0, 65, null, 409, 578], {});
        const referenceCumTimes = [0, 58, 224, 381, 552];
    ...
    QUnit.test("Position after leg returns cumulative ranks", assert => {
        const chartType = ChartTypes.get("PositionAfterLeg");
        const result1 = fromCumTimes(1, 10 * 3600 + 30 * 60, [0, 81, 278, 490, 596], {});
        const result2 = fromCumTimes(2, 10 * 3600, [0, 65, 286, 495, 595], {});
    ...
    QUnit.test("Split position returns split ranks", assert => {
        const chartType = ChartTypes.get("SplitPosition");
        const result1 = fromSplitTimes(1, "Second Runner", "DEF", 10 * 3600 + 30 * 60, [81, 197, 212, 106]);
        const result2 = fromSplitTimes(2, "First Runner", "ABC", 10 * 3600, [65, 221, 209, 100]);
        new CourseClassSet([new CourseClass("Test", 3, [result1, result2])]);
    ...
    QUnit.test("Percent behind returns percents behind the reference time.", assert => {
        const chartType = ChartTypes.get("PercentBehind");
        const result = fromSplitTimes(1, "Second Runner", "DEF", 10 * 3600 + 30 * 60, [81, 197, 212, 106]);
        const referenceCumTimes = [0, 58, 224, 381, 552]; // Splits are 58, 166, 157, 171.

        const expectedData = [0, 100 * (81 - 58) / 58, 100 * (197 - 166) / 166, 100 * (212 - 157) / 157, 100 * (106 - 171) / 171];

        assert.deepEqual(chartType.dataSelector(result, referenceCumTimes), expectedData);
    });

    QUnit.test("All chart types have a name key", assert => {
        for (let chartType of ALL_CHART_TYPES) {
            assert.strictEqual(typeof chartType.nameKey, "string");
        }
    });

    QUnit.test("All chart types except the results table have a y-axis label key", assert => {
        for (let chartType of ALL_CHART_TYPES) {
    ...
    QUnit.test("Only the Race Graph shows the Crossing Runners button", assert => {
        for (let chartType of ALL_CHART_TYPES) {
            assert.strictEqual(chartType.isRaceGraph, chartType === ChartTypes.get("RaceGraph"));
        }
    });

    QUnit.test("Only the Results Table is the Results Table", assert => {
        for (let chartType of ALL_CHART_TYPES) {
            assert.strictEqual(chartType.isResultsTable, chartType === ChartTypes.get("ResultsTable"));
        }
    });

    QUnit.test("All chart types have a minimum viewable control", assert => {
        for (let chartType of ALL_CHART_TYPES) {
            assert.strictEqual(typeof chartType.minViewableControl, "number");
        }
    });

    QUnit.test("Race graph's min viewable control is the start, for all others it's control 1", assert => {
        for (let chartType of ALL_CHART_TYPES) {
            assert.strictEqual(chartType.minViewableControl, (chartType === ChartTypes.get("RaceGraph")) ? 0 : 1);
        }
    });

    QUnit.test("All chart types except the results table have the correct dubious-indexes function", assert => {
        const result = fromOriginalCumTimes(1, 10 * 3600, [0, 96, 96, 96 + 221 + 184, 96 + 221 + 184 + 100], {});
