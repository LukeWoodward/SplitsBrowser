...
(function () {
    ...
    QUnit.test("Parsing single course with no competitors returns single empty course", function (assert) {
        ...
        var actualCourseData = parseEventData(csvData);
        assert.deepEqual(actualCourseData, [new CourseData("Example", 4, [])]);
    });
    ...
    QUnit.test("Can parse a single course with a single valid competitor", function (assert) {
        ...
        var actualCourseData = parseEventData(csvData);
        var expectedCourseData = [new CourseData("Example", 4, [
            new CompetitorData(1, "John", "Smith", "ABC", "10:34", [177, 99, 211, 121, 23])
        ])];
        assert.deepEqual(actualCourseData, expectedCourseData);
    });
    ...
    QUnit.test("Can parse a single course with three valid competitors in correct time order", function (assert) {
        ...
        var actualCourseData = parseEventData(csvData);
        var expectedCourseData = [new CourseData("Example", 4, [
            new CompetitorData(1, "John", "Smith", "ABC", "10:34", [170, 104, 209, 100, 28]),
            new CompetitorData(2, "Fred", "Baker", "DEF", "12:12", [177, 99, 211, 121, 23]),
            new CompetitorData(3, "Jane", "Palmer", "GHI", "11:22", [162, 111, 240, 91, 30])
        ])];
        assert.deepEqual(actualCourseData, expectedCourseData);
    });
    ...
    QUnit.test("Can parse a single course with three valid competitors not in correct time order", function (assert) {
        ...
        var actualCourseData = parseEventData(csvData);
        var expectedCourseData = [new CourseData("Example", 4, [
            new CompetitorData(3, "John", "Smith", "ABC", "10:34", [170, 104, 209, 100, 28]),
            new CompetitorData(1, "Fred", "Baker", "DEF", "12:12", [177, 99, 211, 121, 23]),
            new CompetitorData(2, "Jane", "Palmer", "GHI", "11:22", [162, 111, 240, 91, 30])
        ])];
        assert.deepEqual(actualCourseData, expectedCourseData);
    });
    ...
    QUnit.test("Can parse two courses each with two valid competitors", function (assert) {
        ...
        var actualCourseData = parseEventData(csvData);
        var expectedCourseData = [
            new CourseData("Example", 4, [
                new CompetitorData(1, "John", "Smith", "ABC", "10:34", [177, 99, 211, 121, 23]),
                new CompetitorData(2, "Fred", "Baker", "DEF", "12:12", [162, 111, 240, 91, 30])]),
            new CourseData("Another example course", 5, [
                new CompetitorData(1, "Jane", "Palmer", "GHI", "11:22", [170, 104, 209, 100, 189, 28]),
                new CompetitorData(2, "Faye", "Claidey", "JKL", "10:58", [175, 120, 228, 109, 212, 37])])];
        assert.deepEqual(actualCourseData, expectedCourseData);
    });
    ...
    QUnit.test("Can parse a single course with two valid competitors and one mispuncher in correct order", function (assert) {
        ...
        var expectedCourseData = [new CourseData("Example", 4, [
            new CompetitorData(1, "John", "Smith", "ABC", "10:34", [170, 104, 209, 100, 28]),
            new CompetitorData(2, "Jane", "Palmer", "GHI", "11:22", [177, 99, 211, 121, 23]),
            new CompetitorData(3, "Fred", "Baker", "DEF", "12:12", [162, 111, null, 91, 30])
        ])];
        assert.deepEqual(actualCourseData, expectedCourseData);
    });
    ...
    QUnit.test("Can parse a single course with two valid competitors and one mispuncher not in correct order", function (assert) {
        ...
        var actualCourseData = parseEventData(csvData);
        var expectedCourseData = [new CourseData("Example", 4, [
            new CompetitorData(2, "John", "Smith", "ABC", "10:34", [170, 104, 209, 100, 28]),
            new CompetitorData(3, "Jane", "Palmer", "GHI", "11:22", [177, 99, 211, 121, 23]),
            new CompetitorData(1, "Fred", "Baker", "DEF", "12:12", [162, 111, null, 91, 30])
        ])];
        assert.deepEqual(actualCourseData, expectedCourseData);
    });
    ...
    QUnit.test("Can parse a single course with two valid competitors and two mispunchers not in correct order", function (assert) {
        ...
        var expectedCourseData = [new CourseData("Example", 4, [
            new CompetitorData(2, "John", "Smith", "ABC", "10:34", [170, 104, 209, 100, 28]),
            new CompetitorData(4, "Jane", "Palmer", "GHI", "11:22", [177, 99, 211, 121, 23]),
            new CompetitorData(1, "Fred", "Baker", "DEF", "12:12", [162, 111, null, 91, 30]),
            new CompetitorData(3, "Faye", "Claidey", "JKL", "10:37", [231, null, null, 503, 49])
        ])];
        assert.deepEqual(actualCourseData, expectedCourseData);
    });
