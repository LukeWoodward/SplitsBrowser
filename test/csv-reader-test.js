...
(function () {
    ...
    QUnit.test("Parsing single class with no competitors returns single empty class", function (assert) {

        var csvData = "Example, 4";
        var actualClass = parseEventData(csvData);
        assert.deepEqual(actualClass, [new AgeClass("Example", 4, [])]);
    });
    ...
    QUnit.test("Can parse a single class with a single valid competitor", function (assert) {
        ...
        assert.deepEqual(actualClass, expectedClass);
    });
