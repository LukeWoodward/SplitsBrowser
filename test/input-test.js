...
(function () {
    ...
    QUnit.test("Can read in CSV data", function (assert) {
        ...
        let eventData = parseEventData(csvData);
    ...
    QUnit.test("Can read in OE semicolon-delimited data", function (assert) {
        ...
        let eventData = parseEventData(oeData);
        assert.ok(eventData instanceof Event, "An event should be returned");
    ...
    QUnit.test("Can read in OE CSV data in nameless format", function (assert) {
        let namelessCSVData = "OE0014,Stno,XStno,Chipno,Database Id,Surname,First name,YB,S,Block,nc,Start,Finish,Time,Classifier,Credit -,Penalty +,Comment,Club no.," +
        ...
        let eventData = parseEventData(namelessCSVData);
        assert.ok(eventData instanceof Event, "An event should be returned");
        assert.strictEqual(eventData.classes.length, 1, "One class should be read in");
        assert.strictEqual(eventData.courses.length, 1, "One course should be read in");
    });
    ...
    QUnit.test("Can read in HTML preformatted data", function (assert) {
        let htmlData = '<html><head></head><body>\n<pre>\n<font size="2"><b>   Test course 1 (2)</b></font><font size="2"><b>   2.7 km     35 m</b></font>\n' +
            ...
            "</pre></body></html>";
        let eventData = parseEventData(htmlData);
        assert.ok(eventData instanceof Event, "An event should be returned");
        assert.strictEqual(eventData.classes.length, 1, "One class should be read in");
        assert.strictEqual(eventData.courses.length, 1, "One course should be read in");
    });
    ...
    QUnit.test("Can read in alternative CSV data in triple-column format", function (assert) {
        let tripleColumnCSVData = "RaceNumber,CardNumbers,MembershipNumbers,Name,AgeClass,Club,Country,CourseClass,StartTime,FinishTime,RaceTime,NonCompetitive," +
        ...
        let eventData = parseEventData(tripleColumnCSVData);
        assert.ok(eventData instanceof Event, "An event should be returned");
        assert.strictEqual(eventData.classes.length, 1, "One class should be read in");
        assert.strictEqual(eventData.courses.length, 1, "One course should be read in");
    });
    ...
    QUnit.test("Can read in IOF v2.0.3 XML data", function (assert) {
        ...
        let eventData = parseEventData(iofXmlData);
        assert.ok(eventData instanceof Event, "An event should be returned");
        assert.strictEqual(eventData.classes.length, 1, "One class should be read in");
        assert.strictEqual(eventData.courses.length, 1, "One course should be read in");
    });
    ...
    QUnit.test("Can read in IOF v3.0 XML data", function (assert) {
        let iofXmlData = '<?xml version="1.0" ?><ResultList xmlns="http://www.orienteering.org/datastandard/3.0"\n' +
    ...
    QUnit.test("Cannot read in invalid data", function (assert) {
        let invalidData = "This is not valid results data in any format";
        let result = parseEventData(invalidData);
