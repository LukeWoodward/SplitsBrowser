...
(function () {
    ...
    */
    ...
    QUnit.test("Can create a results table for an empty course-class", function (assert) {
        let resultsTable = new ResultsTable(d3.select("#qunit-fixture").node());
    ...
    QUnit.test("Can create a results table with two results finishing", function (assert) {
        let result1 = fromSplitTimes(1, "First Runner", "DEF", 10 * 3600 + 30 * 60, [65, 221, 184, 100]);
        let result2 = fromSplitTimes(2, "Second Runner", "ABC", 10 * 3600, [81, 197, 212, 106]);
        let courseClass = new CourseClass("Test", 3, [result1, result2]);
        ...
        let topRowCells = $("tbody tr:first-child td", table.node());
        let cum1Cell = $("span:first-child", topRowCells[3]);
        assert.ok(cum1Cell.hasClass("fastest"));
        let split1Cell = $("span:last-child", topRowCells[3]);
        assert.ok(split1Cell.hasClass("fastest"));
        let cum2Cell = $("span:first-child", topRowCells[4]);
        assert.strictEqual(cum2Cell.text(), "04:46");
        assert.ok(!cum2Cell.hasClass("fastest"));
        let split2Cell = $("span:last-child", topRowCells[4]);
    ...
    QUnit.test("Can create a results table with one result not finishing sorted to the bottom", function (assert) {
        let result1 = fromSplitTimes(1, "First Runner", "DEF", 10 * 3600 + 30 * 60, [65, 221, null, 100]);
        let result2 = fromSplitTimes(2, "Second Runner", "ABC", 10 * 3600, [81, 197, 212, 106]);
        let courseClass = new CourseClass("Test", 3, [result1, result2]);
        calculateRanks(courseClass);
        ...
        let lastRow = $("tbody tr:last-child td", table.node());
    ...
    QUnit.test("Can create a results table with one mispunching result", function (assert) {
        let result = fromSplitTimes(1, "First Runner", "DEF", 10 * 3600 + 30 * 60, [65, 221, null, 100]);
        let courseClass = new CourseClass("Test", 3, [result]);
        calculateRanks(courseClass);
        ...
        let row = $("tbody tr:last-child td", table.node());
    ...
    QUnit.test("Can create a results table with one non-starting result", function (assert) {
        let result = fromSplitTimes(1, "First Runner", "DEF", 10 * 3600 + 30 * 60, [null, null, null, null]);
    ...
    QUnit.test("Can create a results table with one non-finishing result", function (assert) {
        let result = fromSplitTimes(1, "First Runner", "DEF", 10 * 3600 + 30 * 60, [65, 221, null, null]);
    ...
    QUnit.test("Can create a results table with one disqualified result", function (assert) {
        let result = fromSplitTimes(1, "First Runner", "DEF", 10 * 3600 + 30 * 60, [65, 221, 184, 100]);
        result.disqualify();
        let courseClass = new CourseClass("Test", 3, [result]);
        calculateRanks(courseClass);
    ...
    QUnit.test("Can create a results table with one result with dubious times appropriately classed", function (assert) {
        let result = fromOriginalCumTimes(1, 10 * 3600 + 30 * 60, [0, 65, 65 + 0, 65 + 221 + 184, 65 + 221 + 184 + 100], new Competitor("First Runner", "DEF"));
        ...
        assert.strictEqual(d3.selectAll("table.resultsTable").size(), 1, "There should be one table");
        let table = d3.select("table.resultsTable").node();
    ...
    QUnit.test("Can create a results table with one result with missing times appropriately classed", function (assert) {
        let result = fromCumTimes(1, 10 * 3600 + 30 * 60, [0, 65, null, 65 + 221 + 184, 65 + 221 + 184 + 100], new Competitor("First Runner", "DEF"));
    ...
    QUnit.test("Can create a results table with one result with fractional times appropriately formatted", function (assert) {
        let result = fromSplitTimes(1, "First Runner", "DEF", 10 * 3600 + 30 * 60, [65.3, 221.0, 184.7, 100.5]);
        let courseClass = new CourseClass("Test", 3, [result]);
        courseClass.setCourse(new Course("Test", [courseClass], 4.1, 140, null));
        calculateRanks(courseClass);
        ...
        let cum2Cell = $("span:first-child", tableCells[4]);
        assert.strictEqual(cum2Cell.text(), "04:46.3");
        let split2Cell = $("span:last-child", tableCells[4]);
        assert.strictEqual(split2Cell.text(), "03:41.0");

        let cum3Cell = $("span:first-child", tableCells[5]);
        assert.strictEqual(cum3Cell.text(), "07:51.0");
        let split3Cell = $("span:last-child", tableCells[5]);
    ...
    QUnit.test("Can create a results table with one result with fractional split times and a finish time with a whole number of seconds appropriately formatted", function (assert) {
        let result = fromSplitTimes(1, "First Runner", "DEF", 10 * 3600 + 30 * 60, [65.3, 221.0, 184.7, 100.0]);
        let courseClass = new CourseClass("Test", 3, [result]);
        courseClass.setCourse(new Course("Test", [courseClass], 4.1, 140, null));
        calculateRanks(courseClass);
        ...
        let cum2Cell = $("span:first-child", tableCells[2]);
    ...
    QUnit.test("Can create a results table with two team results finishing and label the control headers", function (assert) {
        ...
        let team1 = new Team("Team 1", "DEF", [result1a.owner, result1b.owner]);
        let team2 = new Team("Team 2", "ABC", [result2a.owner, result2b.owner]);
        let courseClass = new CourseClass("Test", 7, [createTeamResult(1, [result1a, result1b], team1), createTeamResult(2, [result2a, result2b], team2)]);
        ...
        let controlHeaders = tableHeaders.nodes().slice(3).map(node => node.innerHTML);
    ...
    QUnit.test("Can create a results table with two team results with different numbers of controls per leg and label the control headers", function (assert) {
        let result1a = fromSplitTimes(1, "First Runner", "DEF", 10 * 3600 + 30 * 60, [65, 221, 184, 100]);
        let result2a = fromSplitTimes(2, "Second Runner", "ABC", 10 * 3600, [81, 197, 212, 106]);
        let result1b = fromSplitTimes(1, "Third Runner", "DEF", 10 * 3600 + 570, [78, 234, 103]);
        let result2b = fromSplitTimes(2, "Fourth Runner", "ABC", 10 * 3600 + 596, [88, 192, 111]);
        let team1 = new Team("Team 1", "DEF", [result1a.owner, result1b.owner]);
        let team2 = new Team("Team 2", "ABC", [result2a.owner, result2b.owner]);
        let courseClass = new CourseClass("Test", 3, [createTeamResult(1, [result1a, result1b], team1), createTeamResult(2, [result2a, result2b], team2)]);
    ...
    QUnit.test("Can create a results table with two team results finishing and switch to the first leg", function (assert) {
        ...
        let topOfFirstRow = table.selectAll("tbody tr:first-child td span:first-child").nodes().map(node => node.innerHTML);
        assert.deepEqual(topOfFirstRow, ["First Runner", "09:30", "01:05", "04:46", "07:50", "09:30"]);
        let bottomOfFirstRow = table.selectAll("tbody tr:first-child td span:nth-child(3)").nodes().map(node => node.innerHTML);
