...
(function (){
    ...
    const RESULT_LIST_ID = "resultList";
    ...
    class ResultList {
        constructor(parent, alerter) {
            ...
            this.crossingRunnersButton = this.buttonsPanel.append("button")
                ...
                .style("width", "100%")
                .on("click", () => this.selectCrossingRunners())
                .style("display", "none");
            ...
            this.filter.on("input", () => this.updateFilterIfChanged())
                .on("keyup", () =>  this.updateFilterIfChangedDelayed())
                .on("mouseup", () => this.updateFilterIfChangedDelayed());

            this.listDiv = this.containerDiv.append("div")
                .attr("id", RESULT_LIST_ID);

            this.listDiv.on("mousedown", event => this.startDrag(event, CONTAINER_RESULT_INDEX))
                .on("mousemove", event => this.mouseMove(event, CONTAINER_RESULT_INDEX))
                .on("mouseup", event => this.stopDrag(event));

            d3.select(document.body).on("mouseup", event => this.stopDrag(event));
        ...
        */
        startDrag(event, index) {
            if (event.which === LEFT_BUTTON) {
                ...
                let visibleDivs = this.allResultDivs.filter(":visible");
        ...
        */
        selectionChanged() {
            this.listDiv.selectAll("div.result")
                ...
                .classed("selected", (_result, index) => this.isSelected(index));
        }
        ...
        */
        setResultList(results, multipleClasses, hasTeamData, selectedLegIndex) {
            ...
            */
            ...
            this.allResultDetails = this.allResults.map(result => (
                { result: result, normedName: normaliseName(result.owner.name), visible: true }));
            ...
            let resultDivs = this.listDiv.selectAll("div.result").data(this.allResults);
            ...
            resultDivs.append("span")
                .classed("nonfinisher", result => !result.completed())
                .text(resultText);
            ...
            resultDivs.on("mousedown", (event, index) => this.startDrag(event, index))
                .on("mousemove", (event, index) => this.mouseMove(event, index))
                .on("mouseup", event => this.stopDrag(event));
        ...
        */
        setSelection(selection) {
            ...
            this.handler = () => this.selectionChanged();
        ...
        */
        updateFilter() {
            ...
            this.listDiv.selectAll("div.result")
                .style("display", (_div, index) => (this.allResultDetails[index].visible) ? null : "none");
        }
